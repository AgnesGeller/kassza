...
async function shareViber(){const data=weeklyShareData();let copied=false;try{await navigator.clipboard.writeText(data.text);copied=true;}catch(_){}showShareThanks();if(navigator.share){try{await navigator.share({title:data.subject,text:data.text});$("#shareStatus").textContent=copied?"A teljes heti kassza elküldhető, és a vágólapra is került.":"A teljes heti kassza elküldhető.";return;}catch(error){if(error.name==="AbortError"){if(copied)$("#shareStatus").textContent="A teljes heti kassza a vágólapra került.";return;}}}if(copied)$("#shareStatus").textContent="A teljes heti kassza a vágólapra került. Nyisd meg a Vibert, majd illeszd be.";else $("#shareStatus").textContent="A teljes heti kassza nem másolható automatikusan ezen az eszközön.";location.href="viber://forward";}
