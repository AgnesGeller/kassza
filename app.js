...
async function shareViber(){const data=weeklyShareData();showShareThanks();if(navigator.share){try{await navigator.share({title:data.subject,text:data.text});return;}catch(error){if(error.name==="AbortError")return;}}location.href=`viber://forward?text=${encodeURIComponent(data.text)}`;}
