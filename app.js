...
function updateInvoicePhotoField(){const input=$("#invoicePhoto"),required=$("#receiptField").value==="Számlás"&&!$("#receiptField").disabled;$("#invoicePhotoWrap").hidden=!required;input.disabled=!required;input.required=required;if(!required)input.value="";}
async function shareInvoicePhoto(file,saved){if(!file)return;const status=$("#formStatus"),data={title:`${saved.leader} – számla`,text:`${saved.leader} kasszája · ${formatDate(saved.date)} · ${money(saved.amount)}`,files:[file]};if(navigator.share&&(!navigator.canShare||navigator.canShare({files:[file]}))){try{await navigator.share(data);status.textContent="✓ A tétel elmentve, a számlafotó megosztva.";return;}catch(error){if(error.name==="AbortError"){status.textContent="✓ A tétel elmentve. A fotó küldését megszakítottad.";return;}}}status.textContent="✓ A tétel elmentve. A fotót ezen az eszközön külön kell elküldeni Viberben.";}
...
async function shareViber(){const data=weeklyShareData(),mobile=/Android|iPhone|iPad|iPod/i.test(navigator.userAgent);showShareThanks();if(mobile&&navigator.share){try{await navigator.share({title:data.subject,text:data.text});return;}catch(error){if(error.name==="AbortError")return;}}location.href=`viber://forward?text=${encodeURIComponent(data.text)}`;}
