...
function updateInvoicePhotoField(){const camera=$("#invoiceCamera"),input=$("#invoicePhoto"),invoice=$("#receiptField").value==="Számlás"&&!$("#receiptField").disabled,mobile=isMobileDevice();$("#invoiceCameraWrap").hidden=!invoice||!mobile;$("#invoicePhotoWrap").hidden=!invoice;camera.disabled=!invoice||!mobile;input.disabled=!invoice;camera.required=false;input.required=false;$("#invoicePhotoHelp").textContent=mobile?"Vagy válassz egy meglévő képet a telefon/tablet fájljai közül. Az alkalmazás nem tárolja.":"Válassz egy képfájlt a számítógépről. Az alkalmazás nem tárolja.";if(!invoice){camera.value="";input.value="";}}
